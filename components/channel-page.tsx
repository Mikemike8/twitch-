...
function ChannelContent({ channel, initialFollowing, canFollow, authenticated }: { channel: Channel; initialFollowing: boolean; canFollow: boolean; authenticated: boolean }) {
  ...
  return (
    <div className="flex min-h-screen bg-black pb-16 text-[#efeff1] md:pb-0">
      <section className="flex min-w-0 flex-1 flex-col">
        <div className="relative aspect-video w-full overflow-hidden bg-black lg:min-h-0 lg:flex-1 lg:aspect-auto">
...
function StreamCreatorHeader({ channel, expanded, favorite, onFavorite, onDetails, initialFollowing, authenticated }: { channel: Channel; expanded: boolean; favorite: boolean; onFavorite: () => void; onDetails: () => void; initialFollowing: boolean; authenticated: boolean }) {
  if (expanded) return <div className="flex h-20 shrink-0 items-center gap-2 border-b border-[#29292f] px-3"><Link href="/" className="grid h-9 w-9 shrink-0 place-items-center rounded text-xl text-[#adadb8] hover:bg-[#2f2f35] hover:text-white" aria-label="Return home">←</Link><Avatar channel={channel} /><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{channel.displayName}</strong><i className="mt-0.5 flex items-center gap-1.5 truncate text-[11px] not-italic text-[#d2d2d8]"><span>{formatViewers(channel.viewers)}</span><b className="h-2 w-2 rounded-full bg-red-600" /><span>LIVE</span></i></span><button onClick={onDetails} className="grid h-9 w-7 shrink-0 place-items-center rounded text-2xl text-[#adadb8] hover:bg-[#2f2f35] hover:text-white" aria-label="Open stream details">›</button><FollowButton userId={channel.hostIdentity} initialFollowing={initialFollowing} authenticated={authenticated} compact /><button onClick={onFavorite} className={`grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[#29292f] hover:bg-[#35353d] ${favorite ? "text-[#facc15]" : "text-white"}`} aria-label={favorite ? "Remove from favorites" : "Add to favorites"} title={favorite ? "Favorited" : "Favorite"}><StarIcon className={`h-6 w-6 ${favorite ? "fill-[#facc15]" : ""}`} /></button></div>;

  return <div className="flex h-20 shrink-0 items-center gap-2 border-b border-[#29292f] px-3"><Avatar channel={channel} /><span className="min-w-0 flex-1" /><FollowButton userId={channel.hostIdentity} initialFollowing={initialFollowing} authenticated={authenticated} compact /><button onClick={onFavorite} className={`grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[#29292f] hover:bg-[#35353d] ${favorite ? "text-[#facc15]" : "text-white"}`} aria-label={favorite ? "Remove from favorites" : "Add to favorites"} title={favorite ? "Favorited" : "Favorite"}><StarIcon className={`h-6 w-6 ${favorite ? "fill-[#facc15]" : ""}`} /></button></div>;
}

function PlayerControlsOverlay({ channel }: { channel: Channel }) {
  return <div className="pointer-events-none absolute inset-0 z-[6] bg-black/60"><span className="absolute inset-0 grid place-items-center"><i className="flex gap-2"><b className="h-10 w-3 bg-white" /><b className="h-10 w-3 bg-white" /></i></span><div className="absolute inset-x-0 bottom-0 flex items-center gap-6 bg-gradient-to-t from-black/90 to-transparent pb-5 pl-20 pr-5 pt-14 text-white"><span className="text-xs font-bold">{formatViewers(channel.viewers)} viewers</span><ShareIcon className="ml-auto h-6 w-6" /><FullscreenIcon className="h-6 w-6" /></div></div>;
}
...
function StreamDetails({ channel, canFollow, initialFollowing, authenticated, onClose }: { channel: Channel; canFollow: boolean; initialFollowing: boolean; authenticated: boolean; onClose: () => void }) {
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 p-0 md:items-center md:p-6"><button onClick={onClose} className="absolute inset-0" aria-label="Close stream details" /><section className="relative z-10 w-full max-w-2xl rounded-t-2xl border border-white/10 bg-[#101014] p-6 shadow-2xl md:rounded-2xl md:p-8"><button onClick={onClose} className="absolute right-4 top-4 grid h-10 w-10 place-items-center text-3xl text-white" aria-label="Close stream details">×</button><div className="flex items-center gap-4 pr-10"><Avatar channel={channel} size="lg" /><span className="min-w-0"><strong className="block truncate text-xl font-black">{channel.displayName}</strong><i className="mt-1 block text-sm not-italic text-[#d2d2d8]">{formatViewers(channel.viewers)} watching live</i></span></div><h2 className="mt-7 text-xl font-black leading-tight">{channel.title}</h2><div className="mt-4 flex flex-wrap items-center gap-2"><span className="rounded bg-[#29292f] px-3 py-1.5 text-xs font-bold text-[#efeff1]">{channel.category}</span>{channel.tags.map((tag) => <span key={tag} className="rounded-full bg-[#29292f] px-3 py-1.5 text-xs font-bold text-[#d2d2d8]">{tag}</span>)}</div>{typeof channel.followerCount === "number" && <p className="mt-5 text-xs font-semibold text-[#adadb8]">{channel.followerCount} followers</p>}<p className="mt-4 text-sm leading-6 text-[#adadb8]">{channel.bio || "Welcome to the stream. Follow along for regular broadcasts, live chat, and community events."}</p><div className="mt-7 flex gap-3">{canFollow && <FollowButton userId={channel.hostIdentity} initialFollowing={initialFollowing} authenticated={authenticated} />}<Link href={`/${channel.username}`} className="rounded bg-[#29292f] px-4 py-2 text-sm font-bold hover:bg-[#35353d]">View profile</Link></div></section></div>;
}

...
export function SiteTopbar({
  ...
  return (
    <header className={`${fixed ? "fixed inset-x-0 top-0" : "sticky top-0"} z-40 flex h-[72px] items-center gap-4 border-b border-white/5 ${translucent ? "bg-black/80" : "bg-black/95"} px-8 text-[#f1f1f3] backdrop-blur-xl`}>
      ...
      <nav className="ml-4 hidden items-center gap-1 rounded-full border border-white/10 bg-white/[0.03] p-1 text-xs font-bold uppercase tracking-[0.14em] xl:flex">
      ...
      {clerkConfigured ? (
        <>
          <Show when="signed-out">
            <div className="flex items-center gap-2">
              ...
              <SignUpButton><button className="rounded-lg bg-[#0b5cab] px-3 py-2 text-xs font-bold hover:bg-[#166dca]">Sign up</button></SignUpButton>
            </div>
          </Show>
          <Show when="signed-in">
            <div className="flex items-center gap-2">
              {viewerUsername && (
                <>
                  ...
                  <Link href={`/u/${viewerUsername}`} className="hidden rounded-lg bg-[#0b5cab] px-3 py-2 text-xs font-bold hover:bg-[#166dca] xl:block">Creator Dashboard</Link>
                </>
              )}
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/sign-up" className="hidden rounded-lg bg-[#0b5cab] px-3 py-2 text-xs font-bold hover:bg-[#166dca] sm:block">Sign up</Link>
        </div>
      )}
    </header>
  );

    ...
    <div className="space-y-6 p-4 sm:p-6 lg:p-10">
      ...
      <section className="rounded-lg border border-[#303038] bg-[#18181b] p-5">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Link href={channelHref} className="rounded bg-[#303038] px-3 py-2 text-xs font-bold hover:bg-[#3b3b44]">Open channel</Link>
        </div>
        <div className="mt-5 divide-y divide-[#303038] overflow-hidden rounded-lg border border-[#303038]">
          {participants.length ? participants.map((participant) => (
            <div key={participant.identity} className="flex flex-wrap items-center gap-3 bg-[#111114] p-4">
              ...
              <button disabled={isPending} onClick={() => kick(participant)} className="rounded bg-[#303038] px-3 py-2 text-xs font-bold hover:bg-[#3b3b44] disabled:opacity-50">Kick</button>
              <button disabled={isPending} onClick={() => block(participant)} className="rounded bg-red-600 px-3 py-2 text-xs font-bold hover:bg-red-500 disabled:opacity-50">Block</button>
            </div>
      ...
      <section className="rounded-lg border border-[#303038] bg-[#18181b] p-5">
        ...
        <div className="mt-5 divide-y divide-[#303038] overflow-hidden rounded-lg border border-[#303038]">
          {blockedUsers.length ? blockedUsers.map((user) => (
            <div key={user.id} className="flex flex-wrap items-center gap-3 bg-[#111114] p-4">
              ...
              <button disabled={isPending} onClick={() => unblock(user)} className="rounded bg-[#303038] px-3 py-2 text-xs font-bold hover:bg-[#3b3b44] disabled:opacity-50">Unblock</button>
            </div>

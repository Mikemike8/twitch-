...
export function CommunityList({ initialUsers }: { initialUsers: BlockedUser[] }) {
  ...
  return <div className="mt-6 overflow-hidden rounded-lg border border-[#303038] bg-[#18181b]">{users.map((user) => <div key={user.id} className="flex flex-wrap items-center gap-3 border-b border-[#303038] p-4 last:border-0 sm:gap-4"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#9147ff] text-xs font-black">{user.username.slice(0, 2).toUpperCase()}</div><p className="min-w-0 flex-1 break-words text-sm font-bold">{user.username}</p><button disabled={isPending} onClick={() => startTransition(() => { onUnblock(user.id).then(() => setUsers((current) => current.filter((item) => item.id !== user.id))); })} className="rounded bg-[#303038] px-3 py-2 text-xs font-bold hover:bg-[#3b3b44] disabled:opacity-50">Unblock</button></div>)}</div>;
}
